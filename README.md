# Blue Team AI Detection System v6.0
**決策型檢測系統 - TSAR_RAPTOR_V6**

[![Status](https://img.shields.io/badge/status-decision_oriented-brightgreen)]()
[![Philosophy](https://img.shields.io/badge/philosophy-uncertainty_is_asset-blue)]()
[![Fail_Mode](https://img.shields.io/badge/fail_mode-FAIL_OPEN-green)]()
[![SLO](https://img.shields.io/badge/p95_latency-300ms-orange)]()
...
| 項目 | v4 四層架構 | **v6 決策型系統** |
|------|------------|------------------|
| **系統角色** | 分類器（Classifier） | **資源調度器（Scheduler）** |
| **輸出類型** | Label：`generation_mechanism: "ai"` | **Decision：`routing_decision: "ai_pool"`** |
| **核心指標** | 準確率（Accuracy） | **信息增益/成本比（IG/Cost）** |
| **人工作用** | 標註者（給標籤） | **策略調整者（調參數）** |
| **失敗模式** | Fail Close（有疑慮就停） | **Fail Open（資源不足就降級）** |
| **注意力分配** | 所有樣本平等處理 | **風險梯度排序 + 動態丟棄** |
...
- fail_open_not_fail_close  # 優雅降級而非停機
```

### 新增核心模組

1. **調度器狀態機**（`core/scheduler.py`）
   - 狀態轉移：`INIT → LOW_COST → MID_COST → HIGH_COST → ROUTE`
   - 決策邏輯：基於 `information_gain`（信息增益）
   - 提前終止：`information_gain < 0.10` → STOP

2. **注意力預算治理器**（`core/attention_governor.py`）
   - 三層資源優先級（核心完整性 > 檢測層 > 經濟層）
   - 風險梯度排序：`uncertainty_reduction / cost`
   - GPU 利用率 > 92% 時丟棄底部 20% 樣本

3. **策略學習引擎**（`core/policy_learner.py`）
   - 優化目標：最大化 `uncertainty_reduction_per_cost`
   - 可調閾值：`min_continue`、`disagreement_high`
   - 安全機制：最大變化 0.05，性能退化自動回滾

4. **熔斷器系統**（`core/circuit_breaker.py`）
   - GPU 壓力 > 92% → `DEGRADED_LOW_ONLY`
   - 延遲 > 300ms → `BYPASS`
   - 人工預算耗盡（200/天）→ `DEGRADED_LOW_ONLY`
   - 低信息增益期 → `DEGRADED_LOW_ONLY`

5. **資源降級管理器**（`core/degradation_manager.py`）
   - `NORMAL`：完整流水線
   - `DEGRADED_LOW_ONLY`：僅 LOW_COST 模組
   - `BYPASS`：直接輸出默認路由

6. **人工策略調整器**（`core/human_feedback_processor.py`）
   - 輸出：`hypothesis_adjustment` + `module_trust_feedback`
   - **不再輸出標籤**，僅調整決策策略
...
# v6: 輸出決策（decision）
{
  ...
  "uncertainty_state": 0.23,         # 剩餘不確定性
  "disagreement_score": 0.12,        # 模組分歧度
  "information_gain": 1.35,          # 本次信息增益
  "confidence": None,                # 僅運營指標，非標籤置信度
  "traceable": True,                 # 可回溯
  "reversible": True                 # 可推翻
}
```
...
{
  "video_id": "xxx.mp4",
  "human_label": "real"  # ❌ 給標籤
}

# v6: 人工調整策略（調參數）
{
  "video_id": "xxx.mp4",
  "hypothesis_adjustment": {      # ✅ 調整假說
    "AI": -0.1,
    "Real": +0.1
  },
  "module_trust_feedback": {      # ✅ 調整模組信任度
    "frequency_analyzer": +0.05,
    "model_fingerprint_detector": -0.05
  }
  # 不給標籤，只調策略
}
```
...
基於**第一性原理**、**信息論**、**資源經濟學**構建的決策型檢測系統。

**v6 核心創新**：
- 系統不再輸出「這是 AI」，而是輸出「應該路由到 AI Pool」
- 不確定性不是敵人，而是資產（指導資源分配）
- 計算資源按風險梯度分配，而非平均分配
- 失敗時優雅降級，而非停機
...
**核心變化**：從「分類所有樣本」變為「按信息增益調度資源」

```
┌─────────────────────────────────────────────────────────────┐
│ 【候選影片輸入介面｜Candidate Ingress】                        │
│ • 僅做格式/時間/合法性約束                                     │
│ • ❌ 不做任何 AI/Real 判斷                                    │
└───────────────────────┬─────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────────┐
│ 【生成機制假說層｜Generation Hypothesis】                      │
│ • 輸出：Hypothesis Vector {AI?, Real?, Movie?, Uncertain?}  │
│ • ❌ 不產生 label                                            │
└───────────────────────┬─────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────────┐
│ 【LOW COST 訊號掃描層】                                       │
│ • 模組：frequency_analyzer, texture_noise_detector           │
│ • 輸出：uncertainty_remaining, uncertainty_reduction, IG     │
│ • 決策：IG < 0.10? → STOP (丟棄)                             │
└─────────────┬───────────────────┬───────────────────────────┘
              ↓                   ↓
         STOP (丟棄)          MID COST
              ...
              ┌───────────────────────────────────────────────┐
              │ 【MID COST 衝突放大層】                        │
              │ • 模組：model_fingerprint, physics_violation  │
              │ • 輸出：disagreement_score, updated_uncertainty│
              │ • 決策：U < 0.15 且 D < 0.35? → ROUTE         │
              └─────────┬───────────────┬─────────────────────┘
                        ↓               ↓
                    收斂 → ROUTE    HIGH COST
                                        ↓
              ┌───────────────────────────────────────────────┐
              │ 【HIGH COST 結構分析層】                       │
              │ • 模組：blink_dynamics, facial_rigidity       │
              │ • 僅用於真正困難樣本（max 5% 樣本）            │
              │ • 決策：D >= 0.35? → HUMAN_ESCALATION         │
              └─────────┬───────────────┬─────────────────────┘
                        ↓               ↓
                    ROUTE       HUMAN_ESCALATION
                        ...
                        ↓               ↓
┌─────────────────────────────────────────────────────────────┐
│ 【資料仲裁輸出層｜Decision Only】                              │
│ • 輸出：routing_decision, uncertainty_state, disagreement    │
│ • ❌ 不輸出 label                                            │
│ • ✅ 可回溯、可推翻                                           │
└───────────────────────┬─────────────────────────────────────┘
                        ↓
...
while state not in TERMINAL_STATES:
    if state == "LOW_COST":
        ...
        if result.information_gain < policy.min_continue:  # 0.10
            state = "STOP"  # 丟棄低價值樣本
        else:
            state = "MID_COST"

    elif state == "MID_COST":
        result = run_mid_cost_modules(video)
        if result.information_gain < policy.min_continue:
            state = "STOP"
        elif result.uncertainty_remain < policy.low_exit and \
             result.disagreement < policy.disagreement_high:  # 0.15, 0.35
            state = "ROUTE"  # 收斂，可直接路由
        else:
            state = "HIGH_COST"

    elif state == "HIGH_COST":
        result = run_high_cost_modules(video)
        if result.disagreement >= policy.disagreement_high:  # 0.35
            state = "HUMAN_ESCALATION"  # 需要人工介入
        else:
            state = "ROUTE"

    elif state == "HUMAN_ESCALATION":
        # 人工調整策略，不給標籤
        feedback = human_review(video)
        update_policy(feedback.hypothesis_adjustment,
                     feedback.module_trust_feedback)
        state = "ROUTE"

    elif state == "ROUTE":
        # 輸出決策（而非標籤）
        decision = {
            "routing_decision": determine_pool(result),
            "uncertainty_state": result.uncertainty_remain,
            "disagreement_score": result.disagreement,
            "traceable": True,
            "reversible": True
        }
        break
...
| 特性 | v4 四層架構 | v6 決策型系統 |
|------|------------|--------------|
| **處理策略** | 所有樣本運行所有模組 | 低價值樣本提前終止 |
| **資源分配** | 平均分配 | 按 IG/Cost 排序 |
| **輸出格式** | `label: "ai"` | `routing_decision: "ai_pool"` |
| **人工角色** | 標註困難樣本 | 調整決策策略 |
| **失敗處理** | 停機等待修復 | 自動降級繼續運行 |
| **GPU 治理** | 無限制 | 三層優先級 + 熔斷器 |

---

## 🏗️ 四層架構 Flow Chart（v4 基礎架構）
